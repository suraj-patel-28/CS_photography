...
exports.uploadMedia = async (req, res) => {
  try {
    ...
    const isVideo = req.file.resource_type === 'video' || req.file.mimetype?.startsWith('video/');
    ...
    if (isVideo) {
      ...
      thumbnailUrl = req.file.path.replace('/upload/', '/upload/c_fill,h_400,w_400,q_auto/');
    }
    ...
    if (req.file && (req.file.filename || req.file.public_id)) {
      ...
      await cloudinary.uploader.destroy(req.file.filename || req.file.public_id, { resource_type: resourceType });
    }
...
exports.updateMedia = async (req, res) => {
  try {
    const { title, category, description, featured, order, thumbnailUrl } = req.body;
